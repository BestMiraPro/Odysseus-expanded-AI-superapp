...
def test_dry_run_prints_command_and_does_not_execute(capsys):
    ...
    # run_focus prints shlex.join(command), so an interpreter path containing
    # spaces is quoted. Build the expectation the same way rather than assuming
    # sys.executable has none.
