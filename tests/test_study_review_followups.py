...
def test_stats_counts_question_attempts_not_only_card_reviews(db):
    """The daily chart must reflect practice attempts, which is where most
    retrieval now happens — not card reviews alone."""
    ...
    assert row["attempts_correct"] == 1
    assert out["totals"]["attempts"] == 2


def test_stats_keeps_card_and_question_signals_separate(db):
    """Card recall and question accuracy are different signals, so they are
    reported side by side rather than blended into one rate."""
    ...
    s.add_all([
        ...
        StudyReview(id="r2", owner="alice", card_id="c2", deck_id="dA",
                    rating=3, reviewed_at=now),
        StudyAttempt(id="t1", owner="alice", question_id="a0", deck_id="dA",
                     qtype="mcq", correct=True, attempted_at=now),
        StudyAttempt(id="t2", owner="alice", question_id="a1", deck_id="dA",
                     qtype="mcq", correct=False, attempted_at=now),
    ])
    s.commit()
    ...
    totals = stats_payload("alice", days=7)["totals"]
    assert totals["reviews"] == 2
    assert totals["attempts"] == 2
    # Both cards recalled; one question of two right.
...
def _tagged_attempts(SessionLocal, owner="alice"):
    """Four confidence-tagged attempts in subject dA. Confidence is numeric
    0-100 (Phase 0.3 migrated the old sure/unsure/guess labels to 85/55/30):
    85+right, 85+right, 85+WRONG, 30+right."""
    from core.database import StudyAttempt
    from routes.study_routes import _utcnow_naive
...
def test_calibration_brier_score(db):
    ...
    .85/right .0225 x2, .85/wrong .7225, .30/right .49 -> 1.2575/4."""
    ...
    assert out["overall"]["brier"] == 0.314


def test_calibration_bands_expose_overconfidence(db):
    """Each confidence band must show stated vs actual accuracy — that gap is
    the whole point of asking for a confidence."""
    from routes.study_routes import calibration_payload

    _two_subjects(db)
    _tagged_attempts(db)
    bands = {b["label"]: b for b in calibration_payload("alice", days=90)["buckets"]}

    high = bands["80-99"]
    assert high["attempts"] == 3
    assert high["correct"] == 2
    assert high["accuracy"] == 0.667
    assert high["expected"] == 0.85
    # Claimed 85%, delivered 67% -> overconfident.
    assert high["gap"] == round(0.667 - 0.85, 3)
    assert bands["20-39"]["attempts"] == 1


def test_calibration_sure_but_wrong_rate_per_subject(db):
    """"Sure but wrong" (>= 80% confidence, still wrong) reported per subject."""
    from routes.study_routes import calibration_payload
...
def test_calibration_ignores_untagged_attempts(db):
    """An attempt answered without a confidence cannot be scored."""
    from core.database import StudyAttempt
