...
def test_policy_returns_session_days(tmp_path):
    """This fork ships long-lived sessions (SESSION_TTL_DAYS, default 365)
    rather than upstream's 7-day default, so assert against the configured TTL
    instead of a literal — the point of the test is that policy() reports it."""
