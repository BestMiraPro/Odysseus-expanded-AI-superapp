...
def _process_pdf(path: str, owner: str | None = None,
    ...
    try:
        ...
        logger.warning("PDF processing failed error_type=%s", type(e).__name__)
...
def _process_office_document(
    ...
    ``max_chars=None`` keeps the full text inline (Study uses this so long
    papers aren't silently truncated).
    """
